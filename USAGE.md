...
UNVEIL is a brand new framework used to aid HUMINT practitioners on how to deal with human sources online. More details about how to use the framework can be found online in the book ["The Art and Science of Virtual Humint: A Methodological Proposal for Handling HUMINT Sources Online"](https://unveilframework.net) originally written by [Sofía Sánchez Margolles](https://github.com/sofiasmargolles) and [Félix Brezo Fernández](https://felixbrezo.com).

The book contains most of the theoretical content of the framework. It starts stating the goals and objectives of the UNVEIL framework with an explanation of relevant concepts to be used in the definition of the tactics proposed, including the numbering rules employed. Each tactic contains a specific description for techniques and subtechniques.
...
The principal feature of the Navigator is the ability for users to define layers - custom views of the UNVEIL knowledge
...
our <a href="https://github.com/UNVEILFramework/unveil-navigator/issues">GitHub Issue Tracker</a>. The source code for the
UNVEIL Navigator can be retrieved from our <a href="https://github.com/UNVEILFramework/unveil-navigator">GitHub
repository</a>.
...
versions, the Navigator can manipulate the Enterprise, Mobile, or ICS UNVEIL technology domain knowledge bases, as
well as custom UNVEIL collections or STIX bundles. Within a technology domain, the Navigator allows you to filter 
...
way the Navigator allows you to explore a given UNVEIL matrix and access the definitions of the techniques.
...
file so that UNVEIL data can be used in other applications, analyzed beyond the capability of the UNVEIL Navigator, and
...
layer with the current version of UNVEIL. Past versions of UNVEIL that are compatible with the Navigator are accessible in the 
"More Options" dropdown. This interface allows you to create a new layer from one of the following options:
- Specifying the desired UNVEIL version and domain for the new layer; or
- Specifying the URL of a custom collection or STIX bundle for the new layer. Note: the collection or
...
Saved layer configuration files can be opened in the UNVEIL Navigator to restore a layer or layers you've worked on previously. To
do so, open a new tab and open the "Open Existing Layer" panel. Then click "Upload from local", and select your saved
configuration file. Doing so will restore your saved layer or layers to the UNVEIL Navigator. Layers generated from a custom collection
...
The layer upgrade interface allows users to upgrade an UNVEIL Navigator layer created on a previous version of UNVEIL to the
...
<a href="https://unveilframework.net">the UNVEIL Website</a> for both versions of the technique to enable easy review. In steps with techniques you
...
Each technique in an UNVEIL matrix is tied to one or more platforms. In the Navigator, if you wanted to see only those
techniques in Enterprise UNVEIL which applied to the Linux platform, you would deselect "Windows" and "macOS" under the
...
The UNVEIL Navigator has controls for how the UNVEIL Matrices are displayed. Access controls to change layout via the "
...
will show UNVEIL IDs (e.g "T1000" for techniques, or "TA1000" for tactics) on each cell of the matrix. These controls can
...
The legend helps associate meanings with colors displayed by customized techniques in the UNVEIL Navigator. To open the
...
searched under "search settings:" <i>name</i>, <i>UNVEIL ID</i>, <i>description</i>, and (for techniques) <i>data
...
The UNVEIL Navigator can be customized by modifying the fragment (e.g <code>example.com<b>#fragment</b></code>) of the
URL. A panel on the new tab page exists to build a properly formatted UNVEIL Navigator URL such that, when opened, it
will create an UNVEIL Navigator instance with the desired customizations. This feature may be useful for sharing or
embedding the UNVEIL Navigator.

## Default Layers

Click the "add a layer link" button, then enter a default layer URL pointing to a layer hosted on the web. This will
cause the customized UNVEIL Navigator to initialize with this layer open by default. This is especially useful for
...
The following is an example UNVEIL Navigator URL with the default layer specified to be the *Bear APTs layer
...
Users will not be prompted to upgrade default layers to the current version of UNVEIL if they are outdated.

## Disabling Features

Individual UNVEIL Navigator features can be disabled with the checkboxes. Removing a feature only removes the interface
...
The following is an example UNVEIL Navigator URL with the ability to download the layer and add comments
disabled:  
...
Unlike in previous versions of the Navigator, in more recent versions of the UNVEIL Navigator text size is automatically
...
Copyright 2024 Sofía Sánchez Margolles and Félix Brezo
